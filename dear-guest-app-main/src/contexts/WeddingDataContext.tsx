...
export function WeddingDataProvider({ children }: { children: ReactNode }) {
  ...
  const lastRealtimeEventAt = useRef<number>(Date.now());
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      if (settingsRes.data) {
        const row = settingsRes.data as DB['settings']['Row'];
        const mapped = dbToSettings(row);
      ...
      toast({
        ...
        description: `Something went wrong while loading data: ${String(error)}`,
        variant: 'destructive',
      });
    }
  }, [toast]);
  ...
  useEffect(() => {
    const ensurePolling = () => {
      if (pollIntervalId.current != null) return;

      pollIntervalId.current = window.setInterval(() => {
        const elapsed = Date.now() - lastRealtimeEventAt.current;
        if (elapsed >= 10_000) {
          fetchAll();
        }
      }, 10_000);
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'program_schedule' }, (payload) => {
        lastRealtimeEventAt.current = Date.now();
        const mapRow = (row: any): ProgramItem => ({
          id: row.id,
          time_en: row.time_en,
          time_km: row.time_km,
          title_en: row.title_en,
        ...
        if (payload.eventType === 'INSERT') {
          setProgramSchedule(prev => {
            ...
            return [...prev, mapRow(payload.new)].sort((a, b) => (a.order_index || 0) - (b.order_index || 0));
          });
        } else if (payload.eventType === 'UPDATE') {
          setProgramSchedule(prev => prev
            .map(p => p.id === payload.new.id ? mapRow(payload.new) : p)
      ...
      .subscribe((status) => {
        // If realtime is successfully connected, stop polling.
        if (status === REALTIME_SUBSCRIBE_STATES.SUBSCRIBED) {
          lastRealtimeEventAt.current = Date.now();
          stopPolling();
        }

        // If the connection errors/closes, fall back to polling.
        if (
    ...
    // Start polling until we get our first realtime update.
    ensurePolling();
  ...
  const addGuest = useCallback(async (name: string) => {
    try {
      const { error } = await supabase
        .from('guests')
        .insert({ name, rsvp_status: 'pending', number_of_guests: 1 });
      if (error) throw error;
    } catch (error) {
      console.error('Failed to add guest:', error);
      toast({
        title: 'Unable to add guest',
        description: String((error as any)?.message || error),
        variant: 'destructive',
      });
    }
  ...
  const updateRSVP = useCallback(async (name: string, status: 'attending' | 'not_attending', numGuests: number) => {
    try {
      const { data, error } = await supabase.from('guests').select('id').ilike('name', name).limit(1).single();
      if (error) throw error;

      if (data) {
        const { error: updateError } = await supabase
          .from('guests')
          .update({ rsvp_status: status, number_of_guests: numGuests })
          .eq('id', data.id);
        if (updateError) throw updateError;
      } else {
        const { error: insertError } = await supabase
          .from('guests')
          .insert({ name, rsvp_status: status, number_of_guests: numGuests });
